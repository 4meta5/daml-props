...
Pure state-machine model of the OpenZeppelin [canton-token-template](https://github.com/OpenZeppelin/canton-token-template) AL-10 account-level freeze / compliance hold — the freeze-specific logic of `Rules_SetAccountFrozen` and the unified `assertCanOriginate` origination gate (`assertAccountsNotFrozen`). Models three freeze semantics as invariants over random action sequences (set-frozen / unfreeze / originate): the **origination block** (an origination with a frozen sender or receiver never proceeds — the OFAC/AML hold, INV-40), the **admin carve-out** (the registry administrator can never be set frozen — it co-signs every holding, INV-42), and **non-idempotency** (a redundant freeze change is rejected, keeping the frozen set duplicate-free, INV-42). Following the RoleAdmin/AdminLayer pattern, the fixed executor satisfies every invariant and three buggy executors each drop one guard; the negative tests `test_freezeFrozenOriginationDetected`, `test_freezeAdminFreezeDetected`, and `test_freezeRedundantChangeDetected` each catch the corresponding bypass with a minimal sequence. 5 property tests, all passing. See `DamlProps/Examples/Freeze/`. These rows mirror the account-freeze proofs A14–A18 in the companion [daml-verify](https://github.com/OpenZeppelin/daml-verify) project.
