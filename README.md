...
### canton-token-template role-admin hierarchy (AL-8)

Pure state-machine model of the OpenZeppelin [canton-token-template](https://github.com/OpenZeppelin/canton-token-template) AL-8 delegated role-admin path — `Admin_DelegatedIssueRole`/`Admin_DelegatedRevokeRole` via the `requireRoleAdmin` chokepoint, the `delegableViaRoleAdmin` carve-out / `roleAdmin` relation, and `RoleCapability_Renounce`. Models three semantics as invariants over random action sequences (root-grant / delegated-grant / delegated-revoke / renounce): **role-admin authorization** (granting role R requires holding `roleAdmin R`), the **root carve-out** (the `Admin`/`DEFAULT_ADMIN_ROLE` is never grantable through the delegated path — no re-delegation of root, decision C9), and **renounce self-only** (a party can only renounce a capability that names it). Following the AdminLayer/Amulet pattern, the fixed executor satisfies every invariant and three buggy executors each drop one guard; the negative tests `test_roleAdminEscalationDetected`, `test_roleAdminRootDelegationDetected`, and `test_roleAdminCrossRenounceDetected` each catch the corresponding bypass with a minimal sequence. 5 property tests, all passing. See `DamlProps/Examples/RoleAdmin/`. These rows mirror the role-admin proofs A9–A13 in the companion [daml-verify](https://github.com/OpenZeppelin/daml-verify) project.

### canton-token-template account freeze (AL-10)

Pure state-machine model of the OpenZeppelin [canton-token-template](https://github.com/OpenZeppelin/canton-token-template) AL-10 account-level freeze / compliance hold — `Rules_SetAccountFrozen`, the unified `assertCanOriginate` origination gate over `accountParties` (owner **and** provider), and the cold-path mint accept `Rules_AcceptMintProposal`. **Accounts are modeled with an owner + optional provider** and there is a propose→accept mint flow, so the model exercises the two behaviours the abstract [daml-verify](https://github.com/OpenZeppelin/daml-verify) proofs cannot express. Models four semantics as invariants over random sequences (set-frozen / unfreeze / originate / propose-mint / accept-mint): the **origination block** over owner+provider (INV-40), the **mint-into-frozen block** at cold-path accept (INV-40), the **admin carve-out** (INV-42), and **non-idempotency** (INV-42). The fixed executor computes its gate decision explicitly and records a bypass only when it would *proceed despite* a frozen participant — so the fixed-model test is **non-vacuous** (deleting any fixed guard makes it fail), not just a counter that the buggy executors move. Four buggy executors each drop one guard; the negative tests `test_freezeOwnerOnlyDetected` (a frozen provider, owner-only check — bug #3), `test_freezeMintAcceptDetected` (recipient frozen in the propose→accept window — bug #1), `test_freezeAdminFreezeDetected`, and `test_freezeRedundantChangeDetected` each catch the corresponding bypass with a minimal sequence. 5 property tests, all passing. See `DamlProps/Examples/Freeze/`. Mirrors the account-freeze proofs A14–A17 in daml-verify (which verify the abstract gate logic; the owner/provider and cold-path behaviours are covered here and in the token's daml-script tests).
